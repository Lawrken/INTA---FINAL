...
(function(global){
  ...
  function matchRules(userInput){
    ...
    let recommendations = uniqueRecs.map(name => {
    ...
    // Filter by user's location if specified - only show attractions from their chosen city
    if (ui.location) {
      recommendations = recommendations.filter(r => {
        if (!r.details || !r.details.city) return false; // remove if city unknown
        return r.details.city.toLowerCase() === ui.location;
      });
    }

    // If user chose an interest, keep only attractions whose type matches that interest
    if (ui.interest) {
      recommendations = recommendations.filter(r => {
        if (!r.details || !r.details.type) return true; // keep if we don't know the type
        return r.details.type.toLowerCase() === ui.interest;
      });
    }

    // collect reasons for matched rules
    const ruleReasons = matchedRules
      ...
      .filter(Boolean);
    const reasons = uniqueArray(ruleReasons);

    // compute total stay time hours (sum of durations where known)
    let totalTimeHours = 0;
    for (const r of recommendations) {
      if (r.details && r.details.duration) {
        totalTimeHours += parseDurationToHours(r.details.duration);
      } else {
        // default stay time if unknown: 30 minutes
        totalTimeHours += 0.5;
      }
    }
